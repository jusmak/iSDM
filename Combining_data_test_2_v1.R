...
for (t in 1:length(thinning_PA_opt)) {
  ...
  for (w in 1:length(weights_area_opt)) {
    ...
    for (i in 1:length(species_list)) {
      ...
      #fit models
      source(paste(.wd, "R_code/RunInference_test_2_v1.R", sep = '/'))
      model_fits <- RunInference_v1(.wd, .species, data, thinning, thinning_PA, weights_area,
                                    target_n_obs, m_category)
