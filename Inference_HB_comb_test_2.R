Inference_HB_comb <- function(wd, training_data, PA_data, offset_PO, offset_PA) {
  ...
  beta_samp <- fit_hb_1$par[-2]
  beta <- as.matrix(beta_samp[2:length(beta_samp)])
  ind_pres <- which(training_data$response==1)
  lik_temp <- sum(dpois(training_data$response[ind_pres],
                        exp(beta_samp[1] + x_temp[ind_pres,]%*%beta),
                        log = TRUE))
  
  #extract model parameter estimates
  fit_hb <- list(coefficient = fit_hb_1$par, lik = fit_hb_1$value, lik_po = lik_temp)
  
  return(fit_hb)
}


Inference_HB_single <- function(wd, training_data, PA_data, offset_PO, offset_PA) {
  ...
  f_data <- list("y" = training_data$response/training_data$weights,
                 "x" = x_temp,
                 "N" = length(training_data$response),
                 "linear_sigma" = linear_sigma,
                 "weights" = training_data$weights,
                 "offset" = log(offset_PO[,1]) + log(offset_PO[,2]))
  ...
  #setup the model
  m_hb_1 <- stan_model(file=paste(wd, 'R_code/PPP_covariate.stan', sep = '/'))
