...
save(species_n_data, file = 'Model_fits_v1/Summaries_all_sp/Species_n_data.RData')
...
save(dist_temp, file = 'Model_fits_v1/Summaries_all_sp/Species_dist_data.RData')
...
save(range_size_temp, file = 'Model_fits_v1/Summaries_all_sp/Species_range_data.RData')



#species richness from the range maps 50 km resolution
...
for (i in 1:length(species_list)) {
  ...
  background = aggregate(background, fact=50, fun=max)
...
species_rich = raster()
extent(species_rich) <- extent_max
species_rich_temp = raster(paste0(.wd, '/Data/Range_map_new/', species_list[1], '.tif'))
range_temp = aggregate(range_temp, fact=50, fun=max)
raster::res(species_rich) = raster::res(range_temp)
raster::crs(species_rich) = raster::crs(range_temp)

# get range map for each species
for (i in 1:length(species_list)) {
  .species <- species_list[i]
  species_rich_temp = raster(paste0(.wd, '/Data/Range_map_new/', species_list[i], '.tif'))
  range_temp = aggregate(range_temp, fact=50, fun=max)
  species_rich <- raster::mosaic(species_rich, range_temp, fun = 'sum')
}

save(species_rich, file = 'Model_fits_v1/Summaries_all_sp/Species_richness_range_maps_50km.RData')
...
save(pred_range, file = 'Model_fits_v1/Summaries_all_sp/Species_range_pred_50km.RData')
...
save(dens_range, file = 'Model_fits_v1/Summaries_all_sp/Dens_range_map.RData')
...
save(PO_temp, file = 'Model_fits_v1/Summaries_all_sp/Species_po_data.RData')
