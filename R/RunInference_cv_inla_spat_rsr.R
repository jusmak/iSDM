RunInference_cv = function(wd, species, data_inla, thinning, thinning_PA, weights_area, target_n_obs, model_v) {
  ...
  if(file.exists(paste0('Model_fits_v', model_v, '/INLA_spat_rsr_', species, '_fits_CV_', thin_mark, thin_PA_mark, '_quad_n_',
                       target_n_obs, '_', weights_mark, '_inla.RData'))) {
    
    load(paste0('Model_fits_v', model_v, '/INLA_spat_rsr_', species, '_fits_CV_', thin_mark, thin_PA_mark, '_quad_n_',
               target_n_obs, '_', weights_mark, '_inla.RData'))
  ...
  if (!is.na(temp_i)) {
    ...
    for (i in temp_i:length(data_inla$pa_testing_folds)) {
      ...
      if (length(data_inla_temp$po_response) == 0) {
        ...
        #QR decomposition
        x_temp = cbind(rep(1,length(data_inla_temp$po_response)),data_inla_temp$po_covariates)
        q = qr.Q(qr(x_temp))
        ...
        #covariance matrix of presence-absence points
        pa_A_ref = inla.spde.make.A(mesh = data_inla_temp$spde_mesh, loc = data_inla_temp$pa_coordinates)
        
        # 2.
        #presence points
        stk_po = inla.stack(data = list(y=cbind(rep(1, sum(data_inla_temp$po_response==1)), NA), e=0), A = list(1, po_A_p), tag = 'po',
                             effects = list(list(data.frame(Constant = rep(1, sum(data_inla_temp$po_response==1)), data_inla_temp$po_covariates[data_inla_temp$po_response==1,])),
        ...
        stk_back = inla.stack(data = list(y=cbind(rep(0, sum(data_inla_temp$po_response==0)), NA), e=data_inla_temp$po_weights[data_inla_temp$po_response==0]), A = list(1, po_A_b), tag = 'back',
                               effects = list(list(data.frame(Constant = rep(1, sum(data_inla_temp$po_response==0)), data_inla_temp$po_covariates[data_inla_temp$po_response==0,])),
                                              list(po_spde = 1:full_spde$n.spde)))
        ...
        # 4.
        stk_all = inla.stack(stk_po, stk_back, stk_pa_test, stk_pa_ref)
        
        f1 = as.formula(paste0('y ~ -1 + Constant + ', paste0(colnames(data_inla_temp$po_covariates), collapse = ' + '),
                                " +  f(po_spde, model = full_spde, extraconstr = list(A=as.matrix(t(q)%*%po_A_all), e=rep(0,ncol(x_temp)))) + f(pa_spde_test, copy = 'po_spde', fixed = TRUE)"))
        ...
        #QR decomposition
        x_temp = cbind(rep(1,length(data_inla_temp$pa_response)),data_inla_temp$pa_covariates)
        q = qr.Q(qr(x_temp))
        ...
        # 3.
        stk_pa = inla.stack(data = list(y=data_inla_temp$pa_response, Ntrials=rep(1, length(data_inla_temp$pa_response))), A = list(1, pa_A), tag = 'pa',
                             effects = list(list(data.frame(Constant = rep(1, length(data_inla_temp$pa_response)), data_inla_temp$pa_covariates)),
        ...
        f1 = as.formula(paste0('y ~ -1 + Constant + ', paste0(colnames(data_inla_temp$po_covariates), collapse = ' + '),
                                " +  f(pa_spde, model = full_spde, extraconstr = list(A=as.matrix(t(q)%*%pa_A), e=rep(0,ncol(x_temp)))) + f(pa_spde_test, copy = 'pa_spde', fixed = TRUE)"))
        
        # 5.
        ft.inla = 0
        ft.inla = try(inla(f1, family = 'binomial', data = inla.stack.data(stk_all, spde = full_spde),
                       control.fixed = list(prec = list(default=0.1), prec.intercept = list(default=0.1)),
                       control.family = list(list(link = 'cloglog')),
                       control.predictor = list(A = inla.stack.A(stk_all), compute = TRUE),
                       Ntrials = inla.stack.data(stk_all)$Ntrials,
                       control.compute = list(dic = TRUE),
                       verbose = TRUE))
        ...
        #QR decomposition
        x_temp = cbind(rep(1,(length(data_inla_temp$po_response)+length(data_inla_temp$pa_response))),rbind(data_inla_temp$po_covariates, data_inla_temp$pa_covariates))
        q = qr.Q(qr(x_temp))
        ...
        #combine covariance matrices for the contraining of the spatial random effect
        p_comb_A_all = rbind(po_A_p, po_A_b, pa_A)
        ...
        f1 = as.formula(paste0('y ~ -1 + ConstantA + ConstantB + ', paste0(colnames(data_inla_temp$po_covariates), collapse = ' + '),
                                " +  f(po_spde, model = full_spde, extraconstr = list(A=as.matrix(t(q)%*%p_comb_A_all), e=rep(0,ncol(x_temp)))) +  f(pa_spde, copy = 'po_spde', fixed = TRUE) +  f(pa_spde_test, copy = 'po_spde', fixed = TRUE)"))
      ...
      save(output_cv, file = paste0('Model_fits_v', model_v, '/INLA_spat_rsr_', species, '_fits_CV_', thin_mark, thin_PA_mark, '_quad_n_',
                                   target_n_obs, '_', weights_mark, '_inla.RData'))
