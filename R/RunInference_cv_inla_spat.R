RunInference_cv = function(wd, species, data_inla, thinning, thinning_PA, weights_area, target_n_obs, model_v) {
  ...
  if(file.exists(paste0('Model_fits_v', model_v, '/INLA_spat_', species, '_fits_CV_', thin_mark, thin_PA_mark, '_quad_n_',
                       target_n_obs, '_', weights_mark, '_inla.RData'))) {
    
    load(paste0('Model_fits_v', model_v, '/INLA_spat_', species, '_fits_CV_', thin_mark, thin_PA_mark, '_quad_n_',
               target_n_obs, '_', weights_mark, '_inla.RData'))
  ...
  if (!is.na(temp_i)) {
    ...
    for (i in temp_i:length(data_inla$pa_testing_folds)) {
      ...
      model_titles = c('spat_po', 'spat_pa', 'spat_comb')
      ...
      if (sum(data_inla_temp$po_response) == 0) {
        ...
        #covariance matrix of reference presence-absence points
        ...
        f1 = as.formula(paste0('y ~ -1 + Constant + ', paste0(colnames(data_inla_temp$po_covariates), collapse = ' + '),
                                " +  f(po_spde, model = full_spde) +  f(pa_spde_test, copy = 'po_spde', fixed = TRUE)"))
        ...
        stk_pa_test = inla.stack(data = list(y=rep(NA, length(data_inla_temp$pa_response_test)), Ntrials=rep(1, length(data_inla_temp$pa_response_test))),
                                  A = list(1, pa_A_test), tag = 'pa_test',
                                  effects = list(list(data.frame(Constant = rep(1, length(data_inla_temp$pa_response_test)), data_inla_temp$pa_covariates_test)),
                                            list(pa_spde_test = 1:full_spde$n.spde)))
        ...
        f1 = as.formula(paste0('y ~ -1 + Constant + ', paste0(colnames(data_inla_temp$po_covariates), collapse = ' + '),
                                " +  f(pa_spde, model = full_spde) +  f(pa_spde_test, copy = 'pa_spde', fixed = TRUE)"))
        ...
        f1 = as.formula(paste0('y ~ -1 + ConstantA + ConstantB + ', paste0(colnames(data_inla_temp$po_covariates), collapse = ' + '),
                                " +  f(po_spde, model = full_spde) +  f(pa_spde, copy = 'po_spde', fixed = TRUE) +  f(pa_spde_test, copy = 'po_spde', fixed = TRUE)"))
      ...
      save(output_cv, file = paste0('Model_fits_v', model_v, '/INLA_spat_', species, '_fits_CV_', thin_mark, thin_PA_mark, '_quad_n_',
                                   target_n_obs, '_', weights_mark, '_inla.RData'))
