RunInference = function(wd, species, data_inla, thinning, thinning_PA, weights_area, target_n_obs, model_v) {
  ...
  if(file.exists(paste0('Model_fits_v', model_v, '/INLA_spat_', species, '_fits_', thin_mark, thin_PA_mark, '_quad_n_',
                       target_n_obs, '_', weights_mark, '_inla.RData'))) {
    
    load(paste0('Model_fits_v', model_v, '/INLA_spat_', species, '_fits_', thin_mark, thin_PA_mark, '_quad_n_',
               target_n_obs, '_', weights_mark, '_inla.RData'))
    ...
    f1 = as.formula(paste0('y ~ -1 + Constant + ', paste0(colnames(data_inla$po_covariates), collapse = ' + '),
                            ' +  f(po_spde, model = full_spde)'))
    
    # 5.
    ft.inla = 0
    ft.inla = try(inla(f1, family = 'poisson', data = inla.stack.data(stk_all, spde = full_spde),
    ...
    f1 = as.formula(paste0('y ~ -1 + ConstantA + ConstantB + ', paste0(colnames(data_inla$po_covariates), collapse = ' + '),
                            " +  f(po_spde, model = full_spde) +  f(pa_spde, copy = 'po_spde', fixed = TRUE)"))
    ...
    save(output, file = paste0('Model_fits_v', model_v, '/INLA_spat_', species, '_fits_', thin_mark, thin_PA_mark, '_quad_n_',
                                  target_n_obs, '_', weights_mark, '_inla.RData'))
