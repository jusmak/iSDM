...
#species
species_list = c('Ocreatus_underwoodii',
                 'Ensifera_ensifera',
...
#models are run with three different methods for balancing the impact of differing
#number of data points on the importance of likelihood functions:
#1. no scaling of likelihood
#2. likelihood is scaled on an arithmetic scale
#3. likelihood is scaled on a logarithmic scale
...
for (t in 1:length(thinning_PA_opt)) {
  ...
  for (w in 1:length(weights_area_opt)) {
    ...
    for (i in 1:length(species_list)) {
      ...
      n_fold <- spatialStratify(occ_sp_points, sp_stats, nfolds = 4, nsubclusters = 4)
      ...
      #check which fold is closest to have one fourth of presence records
      pres_temp <- 0
      for (j in 1:length(ind_fold)) {
        pres_temp[j] <- sum(data$inventory_data$response[ind_fold[[j]]])-
          sum(data$inventory_data$response)/length(ind_fold)
      }
      
      ind_test <- ind_fold[[which.min(abs(pres_temp))]]
      ind_train <- 0
      for (j in (1:length(ind_fold))[-which.min(abs(pres_temp))]) {
        ind_train <- c(ind_train, ind_fold[[j]])
      }
      ind_train <- ind_train[-1]
      
      # subsample inventory data for training
      data$inventory_data$coordinates <- data$inventory_data$coordinates[ind_train,]
      data$inventory_data$covariates <- data$inventory_data$covariates[ind_train,]
      data$inventory_data$response <- data$inventory_data$response[ind_train]
      for (j in 1:length(data$inventory_data$offset)) {
        data$inventory_data$offset[[j]] <- data$inventory_data$offset[[j]][ind_train,]
      }
      
      # subsample inventory data for testing
      data$pred_PA_data$coordinates <- data$pred_PA_data$coordinates[ind_test,]
      data$pred_PA_data$covariates <- data$pred_PA_data$covariates[ind_test,]
      data$pred_PA_data$response <- data$pred_PA_data$response[ind_test]
      for (j in 1:length(data$pred_PA_data$offset)) {
        data$pred_PA_data$offset[[j]] <- data$pred_PA_data$offset[[j]][ind_test,]
      }
      ...
      for (j in 1:3) {
        ...
        #fit models
        source(paste(.wd, "R_code/RunInference_test_1_v1.R", sep = '/'))
        model_fits <- RunInference_v1(.wd, .species, data, thinning, thinning_PA, weights_area, target_n_obs, m_category)
        
        #compute predictive accuracy of the models
        source(paste(.wd, "R_code/Predictive_tests_test_1.R", sep = '/'))
        pred_metric <- Predictive_tests(.wd, model_fits, .species, data$training_data, data$offset_full,
                                        data$pred_PA_data, data$proj_raster, thinning, thinning_PA,
                                        weights_area, target_n_obs, m_category, n_samp)
      }
    }
  }
}
