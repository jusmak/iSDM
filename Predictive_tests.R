Predictive_tests <- function(wd, model_fits, species, pred_PA_data) {
  ...
  HB_cv_valid <- Inference_HB_cv_x_fold(wd, model_fits$HB_model, model_fits$training_data, model_fits$offset, ind_fold)

  ##predictive density in the independent validation points
  source(paste(wd, "R_code/Inference_HB_ind_validation.R", sep = '/'))
  HB_ind_valid <- Inference_HB_ind_validation(wd, model_fits, pred_PA_data)
  ...
  glmnet_cv_valid <- Inference_glmnet_cv_x_fold(wd, model_fits$training_data, model_fits$offset, ind_fold)
  ...
  predictive_validation <- list(HB_cv_valid, HB_ind_valid, glmnet_cv_valid, glmnet_ind_valid)
  names(predictive_validation) <- c('HB_cv_valid', 'HB_ind_valid', 'glmnet_cv_valid', 'glmnet_ind_valid')
