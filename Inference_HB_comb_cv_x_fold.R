Inference_HB_cv_x_fold <- function(wd, model_fits, training_data, offset_full, ind_fold, pred_PA_data) {
  ...
  for (i in 1:length(offset_full)) {
    ...
    for (j in 1:length(ind_fold)) {
      ...
      x_temp_inv <- cbind(pred_PA_data$covariates,pred_PA_data$covariates^2)
      
      #compile data for inference
      f_data <- list("y" = resp_temp/weights_temp, "x" = x_temp,
                     "y_inv" = pred_PA_data$response, "x_inv" = x_temp_inv,
                     "N" = length(resp_temp),"N_inv" = length(pred_PA_data$response),
                     "linear_sigma" = linear_sigma,
                     "weights" = weights_temp,
                     "offset" = log(offset_temp[,1]) + log(offset_temp[,2]),
                     "offset_inv" = log(pred_PA_data$offset[[i]][,1]) + log(pred_PA_data$offset[[i]][,2]))

      #setup the model
      m_hb_1 <- stan_model(file=paste(wd, 'R_code/PPP_covariate_comb.stan', sep = '/'))
      
      #take point estimates of the parameters
      fit_hb_1 <- optimizing(object = m_hb_1, data=f_data, iter = 2000, hessian = TRUE, init = inits_chain1,
                             draws = 1000, importance_resampling = TRUE, verbose = TRUE)
      
      fit_1 <- list(coefficient = fit_hb_1$par, lik = fit_hb_1$value)
      model_fits_cv_offset[[j]] <- fit_1
      ...
      alpha <- fit_hb_1$par[1]
      beta <- fit_hb_1$par[3:length(fit_hb_1$par)]
      pred_validation <- matrix(rep(alpha, nrow(pred_matrix)), nrow = nrow(pred_matrix), byrow = TRUE) + pred_matrix %*% beta
      ...
      for (k in 1:length(offset_full)) {
        ...
        #predictive density
        pred_dens <- dpois(resp_temp_test, exp(offset_temp_test_pred + pred_validation + log(weights_temp_test)),
                                    log = TRUE)
        #AUC
        suppressMessages(
          myRoc <- pROC::roc(response = resp_temp_test, predictor = array(1-exp(-exp(offset_temp_test_pred + pred_validation + log(weights_temp_test))))))
