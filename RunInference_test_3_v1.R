RunInference_v1 <- function(wd, species, data, thinning, thinning_PA, weights_area, target_n_obs, m_category) {
  ...
  if(file.exists(paste(wd, "/Model_fits_test_3/", species, '_fits_', thin_mark, thin_PA_mark, "_quad_n_",
                       target_n_obs, "_", weights_mark, '_m_cat_', m_category, '.RData', sep = ''))) {
    
    load(paste(wd, "/Model_fits_test_3/", species, '_fits_', thin_mark, thin_PA_mark, "_quad_n_",
               target_n_obs, "_", weights_mark, '_m_cat_', m_category, '.RData', sep = ''))
  } else {
    
    fit_hb_comb_all <- list()
    source(paste(wd, "R_code/Inference_HB_comb_test_3.R", sep = '/'))
    ...
    for (j in 1:length(weights_temp_PA)) {
      ...
      fit_hb <- Inference_HB_comb(wd, data_temp$training_data, data_temp$inventory_data,
                                  data_temp$offset_full[[1]], data_temp$inventory_data$offset[[1]],j)
    ...
    save(model_fits, file = paste(wd, "/Model_fits_test_3/", species, '_fits_', thin_mark, thin_PA_mark, "_quad_n_",
                                  target_n_obs, "_", weights_mark, '_m_cat_', m_category, '.RData', sep = ''))
  }
  return(model_fits)
}